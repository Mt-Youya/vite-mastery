...
export async function ExamplesBento({ locale }: ExamplesBentoProps) {
  const t = await getTranslations({ locale, namespace: "home.examplesBento" })
  const tExamples = await getTranslations({ locale, namespace: "examplesData" })
    ...
    <section className="border-t border-border bg-bg-subtle/30">
      <div className="mx-auto max-w-doc px-4 py-16 sm:px-6 sm:py-24 md:py-28">
        <header className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          ...
          <Link
            href={localizedHref("/examples", locale)}
        ...
        >
          {EXAMPLES.map((ex, i) => {
            ...
            return (
              <Link
                key={ex.id}
                href={resolveDocHref(ex.chapter, locale)}
                className={cn(
