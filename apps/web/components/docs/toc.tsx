...
export function Toc({ headings, title }: TocProps) {
  ...
  return (
    <nav aria-label={title} className="text-sm">
      <p className="mb-3 font-display text-xs font-semibold tracking-wider text-fg-subtle uppercase">{title}</p>
            ...
            <li key={h.id} style={{ paddingLeft: h.level === 3 ? "1.25rem" : "0.75rem" }}>
              <a
                ...
                className={cn(
                  ...
                  active ? "border-primary font-medium text-fg" : "border-transparent text-fg-muted hover:text-fg"
                )}
              >
                {h.text}
              </a>
