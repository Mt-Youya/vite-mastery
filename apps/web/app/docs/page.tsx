...
export default function DocsIndexPage() {
  ...
  return (
    <article className="max-w-3xl">
      ...
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight text-balance">从这里开始读</h1>
          ...
          return (
            <li key={part.id} className="py-5">
              <Link href={target} className="grid items-baseline gap-2 md:grid-cols-[5rem_1fr_auto] md:gap-6">
                <span className="font-mono text-xs tracking-wider text-fg-subtle tabular-nums">PART {part.no}</span>
                <span>
                  <span className="block font-medium text-fg">{part.title}</span>
                  <span className="block text-sm text-fg-muted text-pretty">{partMeta?.summary}</span>
                </span>
