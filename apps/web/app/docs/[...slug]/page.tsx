...
import { buildDocsTree, findDocWithSiblings, flattenDocs, getBreadcrumb, type DocItem } from "@/lib/docs-tree"
        ...
        <article className="min-w-0">
          ...
          <header className="mt-4">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <VersionBadge version={current.viteVersion} />
              <StabilityBadge stability={current.apiStability as Stability} />
            </div>
          </header>
  ...
  if (part && slug.length === 1) {
    ...
    return (
      <article className="max-w-3xl">
        <Breadcrumb items={[{ label: "文档", href: "/docs" }, { label: `Part ${part.no} · ${part.title}` }]} />
        <header className="mt-4">
          <p className="font-mono text-xs tracking-wider text-fg-subtle">PART {part.no}</p>
          <h1 className="mt-2 font-display text-3xl font-semibold tracking-tight text-balance md:text-4xl">
            {part.title}
          </h1>
          <p className="mt-3 text-base leading-relaxed text-pretty text-fg-muted">{part.summary}</p>
        </header>
