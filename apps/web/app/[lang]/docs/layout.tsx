...
import { buildDocsNavTree, pickDocsForLocale, type DocItem } from "@/lib/docs-tree"
...
export default async function DocsLayout({ children, params }: DocsLayoutProps) {
  ...
  const navTree = buildDocsNavTree(docs)
    ...
    <div className="mx-auto grid max-w-doc gap-8 px-4 py-6 sm:px-6 sm:py-10 md:grid-cols-[16rem_minmax(0,1fr)] lg:gap-12">
      ...
      <aside className="hidden md:block">
        <div className="sticky top-20 max-h-[calc(100dvh-6rem)] overflow-y-auto pr-2">
          <Sidebar tree={navTree} locale={lang} />
        </div>
      </aside>
      <div className="min-w-0">
        <MobileSidebar tree={navTree} locale={lang} />
        {children}
      </div>
    </div>
  )
