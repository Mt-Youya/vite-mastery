...
<Callout type="note" title="What you'll learn">
  - Why file-system routing is a natural fit for virtual modules - How to scan `src/pages` into a stable route table -
  How file add/delete/rename events should invalidate the generated route module - Which edge cases real routing plugins
  handle beyond the teaching version
</Callout>

<Callout type="tip" title="Prerequisites">
  - Read [3.4 · Virtual Modules](/docs/03-plugin-system/04-virtual-modules-plugin) - Read [6.3 ·
  `handleHotUpdate`](/docs/06-hmr/03-handle-hot-update) - Understand the Vite dev server module graph and file watcher
</Callout>
...
<Callout type="warning" title="High-frequency bugs in file-routing plugins">
  - Generated code emits OS absolute paths that the browser cannot import - Windows paths are not normalized, so route
  parsing breaks - New files do not update routes because only old files were watched - Dynamic routes sort before
  static routes, so `/:id` captures `/settings` - The virtual module has no TypeScript declaration - Deleting a page
  does not invalidate the route module, leaving stale routes behind
</Callout>
