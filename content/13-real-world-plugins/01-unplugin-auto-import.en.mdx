...
<Callout type="warning" title="Common auto-import plugin pitfalls">
  - Regex scanning treats object properties, strings, or comments as variable usage - Failing to exclude existing
  imports generates duplicate imports - Failing to detect type-only usage turns type references into runtime imports -
  Stale `.d.ts` output makes editor hints disagree with actual transforms - Huge barrel presets make every file scan too
  many candidate APIs and slow down HMR
</Callout>
