...
<Callout type="note" title="What you'll learn">
  - What `@vitejs/plugin-react` v6 configures in Vite 8 - Why Fast Refresh runtime and HTML preamble are separate
  concerns - Why SSR/backend-integrated apps may need `@vitejs/plugin-react/preamble` - How to migrate Babel-dependent
  v5 setups to v6
</Callout>

<Callout type="tip" title="Prerequisites">
  - Read [4.3 · `transform` Hook](/docs/04-hooks-deep-dive/03-build-hooks-2) - Read [6.5 · React Fast
  Refresh](/docs/06-hmr/05-framework-hmr) - Understand [7.8 · Backend
  Integration](/docs/07-build-pipeline/08-backend-integration)
</Callout>
...
<Callout type="warning" title="Common React plugin migration pitfalls">
  - SSR or backend integration skips `transformIndexHtml`, so the preamble is never installed - A v5 migration keeps
  writing `react({babel})`, but v6 expects `@rolldown/plugin-babel` composition - MDX files are not added to `include`,
  so they never receive the Fast Refresh wrapper - A file mixes component exports with mutable non-component exports,
  forcing Fast Refresh to invalidate - Module federation setups omit `reactRefreshHost`, causing multiple refresh
  runtimes to conflict
</Callout>
