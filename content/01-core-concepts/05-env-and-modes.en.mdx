...
<Callout type="note" title="What you will learn">
  - Built-in constants on `import.meta.env` - `.env`, `.env.local`, and `.env.[mode]` priority - Why only `VITE_`
  variables are exposed to browser code - What `--mode`, `NODE_ENV`, and `loadEnv` are for - How to add TypeScript
  completion for custom variables
</Callout>
