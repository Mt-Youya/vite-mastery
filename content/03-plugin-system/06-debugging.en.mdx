...
<Callout type="note" title="What you'll learn">
  - How to use `@vitejs/devtools` - What the `DEBUG=vite:*` environment variable reveals - The core features of the
  `vite-plugin-inspect` plugin - A systematic checklist for debugging hooks that don't fire
</Callout>
...
<Callout type="warning">
  `@vitejs/devtools` is still an early preview. Treat [devtools.vite.dev/guide/](https://devtools.vite.dev/guide/) as
  the source of truth for commands and integration APIs. For production debugging, keep using `DEBUG=vite:*`, minimal
  reproductions, and build logs alongside visual panels.
</Callout>
