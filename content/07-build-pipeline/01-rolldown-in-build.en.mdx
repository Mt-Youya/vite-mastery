...
<Callout type="note" title="What you will learn">
  - How `vite build` invokes Rolldown in Vite 8 - Why new config should use `build.rolldownOptions` - What
  `build.rollupOptions` means during migration - How Oxc, Lightning CSS, manifests, and chunk import maps fit into the
  build
</Callout>

<Callout type="tip" title="Prerequisites">
  - Read [2.2 · What is Rolldown](/docs/02-bundler-evolution/02-what-is-rolldown) - Understand [3.1 · Vite Plugins vs
  Rollup Plugins](/docs/03-plugin-system/01-vite-vs-rollup-plugin)
</Callout>
...
<Callout type="warning" title="Migration note">
  During migration, keeping `rollupOptions` can help you get a build passing first. After validation, rename it to
  `rolldownOptions` and review each output/plugin option.
</Callout>
