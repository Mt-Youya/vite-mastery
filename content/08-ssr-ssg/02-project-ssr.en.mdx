...
<Callout type="warning" title="Environment API is RC">
  The official SSR guide still shows `vite.ssrLoadModule()` for low-level SSR, while the changes documentation points
  toward `ModuleRunner.import()` in the Environment API model. This project keeps the compatible implementation but
  isolates the loading layer so it can be replaced later.
</Callout>

<Callout type="note" title="What you will learn">
  - The files required for a minimal SSR app - Why dev mode uses Vite middleware - Why production builds client and
  server outputs separately - How `ssrLoadModule` differs from `ModuleRunner.import()` - How to verify SSR, hydration,
  data injection, and stack traces
</Callout>

<Callout type="tip" title="Prerequisites">
  - Read [8.1 · SSR API in the Vite 8 Era](/docs/08-ssr-ssg/01-vite8-ssr-api) - Understand React `renderToString` and
  `hydrateRoot` - Know basic Node.js HTTP servers
</Callout>
