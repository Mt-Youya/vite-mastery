...
<Callout type="note" title="What you will learn">
  - The difference between `create-vite` and the project-local `vite` CLI - What `dev`, `build`, and `preview` actually
  validate - When to use `--host`, `--mode`, `--debug`, and `--force` - How to turn one-off commands into reusable team
  scripts
</Callout>
...
<Callout type="warning">
  `--host 0.0.0.0` exposes the dev server to your local network. Avoid it on untrusted networks, especially when
  proxying sensitive APIs.
</Callout>
