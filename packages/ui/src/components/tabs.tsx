...
function List({ className, ...rest }: ComponentPropsWithoutRef<typeof Base.List>) {
  return <Base.List className={cn("inline-flex items-center gap-1 border-b border-border", className)} {...rest} />
}
...
function Panel({ className, ...rest }: ComponentPropsWithoutRef<typeof Base.Panel>) {
  return (
    <Base.Panel
      className={cn("pt-4 focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-focus)]", className)}
      {...rest}
    />
  )
}
