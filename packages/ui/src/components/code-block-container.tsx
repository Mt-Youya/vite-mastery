      ...
      {hasHeader ? (
        <div className="flex items-center justify-between gap-2 border-b border-border bg-bg-subtle px-3 py-1.5">
          <div className="flex items-center gap-2 font-mono text-xs text-fg-muted">
            {filename && <span className="text-fg">{filename}</span>}
            {lang && (
              <span className="rounded bg-brand-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-brand-700 dark:bg-brand-400/15 dark:text-brand-300">
                {lang}
              </span>
            )}
          </div>
