...
function Trigger({ className, placeholder, children, ...rest }: TriggerProps) {
  ...
  return (
    ...
    >
      {children ?? (
        <Base.Value>
          {(value) => (value === null || value === undefined || value === "" ? placeholderText : String(value))}
        </Base.Value>
      )}
      <HugeiconsIcon icon={ArrowDown01Icon} className="size-4 text-fg-muted" strokeWidth={1.5} aria-hidden />
    </Base.Trigger>
  )
}
  ...
  return (
    <Base.Portal>
      <Base.Positioner sideOffset={sideOffset} className="z-50">
        <Base.Popup
          className={cn(
            "max-h-[min(60vh,24rem)] min-w-(--anchor-width) overflow-y-auto",
  ...
  return (
    ...
    >
      <span className="absolute left-2 flex size-4 items-center justify-center">
        <Base.ItemIndicator>
          <HugeiconsIcon icon={Tick02Icon} className="size-3.5 text-primary" strokeWidth={1.5} aria-hidden />
        </Base.ItemIndicator>
      </span>
