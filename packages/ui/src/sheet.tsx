...
const SIDE_CLASSES: Record<Side, string> = {
  ...
  top: "top-0 left-0 w-full max-h-[80vh] border-b data-[state=open]:slide-in-from-top data-[state=closed]:slide-out-to-top",
  bottom:
...
function Content({ side = "right", title, description, closable = true, className, children, ...rest }: ContentProps) {
  return (
    <Base.Portal>
      ...
      >
        <div className="flex items-start justify-between gap-4 pb-4">
          <div className="space-y-1">
            ...
            {description ? <Base.Description className="text-sm text-fg-muted">{description}</Base.Description> : null}
          </div>
          {closable ? (
            <Base.Close
              render={
                <IconButton aria-label="关闭" size="sm" variant="ghost">
                  <HugeiconsIcon icon={Cancel01Icon} className="size-4" strokeWidth={1.5} aria-hidden />
                </IconButton>
              }
            />
          ) : null}
