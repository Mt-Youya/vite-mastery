...
vite-mastery 内部组件库。**底层 primitive = Base UI**(`@base-ui-components/react`),设计 token 来自 `@vite-mastery/tailwind-config`。
...
| 原子 | `Switch` `Toggle` `Separator` | `@base-ui-components/react/switch` 等 |
| 复合 | `Dialog` `Sheet` | `@base-ui-components/react/dialog` |
| 复合 | `Popover` `Tooltip` | 同名子包 |
| 复合 | `Tabs` `Select` `Combobox` | 同名子包(Combobox = AutoComplete) |
| 复合 | `ScrollArea` | `@base-ui-components/react/scroll-area` |
...
| 包 | 版本 |
|---|---|
| `@base-ui-components/react` | `^1.0.0-beta`(API 仍在演进,见各组件注释) |
| `react` | `^19.0.0`(peer) |
